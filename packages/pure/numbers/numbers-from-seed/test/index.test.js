...
describe("numbersFromSeed", () => {
    ...
    it("has valid documentation", () => {
        const source = readFileSync(new URL("../sec/index.ts", import.meta.url), "utf8");
        ...
        if (/\bTODO\b/i.test(documentation)) {
            throw new Error('Invalid documentation for "numbersFromSeed": replace the TODO placeholder with a description of the function.');
        }
    });
    // ---------------------------------- START YOUR TESTS HERE --------------------------------------
    it("generates a deterministic sequence from a seed", () => {
        expect(numbersFromSeed({
            count: 5,
            seed: "abc",
        })).toEqual([1, 1, 0, 1, 1]);
        expect(numbersFromSeed({
